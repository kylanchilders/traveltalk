...
// this is our ajax call to EventBrite
// we will be passing the latitude and longitude retured from Brandon's google ajax call as well as our set locations
function populateEventsByLatLong(latitude, longitude) {

    // here we are declaring a valriable called eventURL passingout longitude and latitude through the url
    ...
    $.ajax({
        // here we are declaring the url will be our eventURL
        url: eventURL,
        // we will be using the "GET" method within ajax
        // "GET" returns information from their database
        method: "GET"
        // this is our promis to our ajax call passing our function(response){} through it
    }).then(function(response){

        console.log("ajax return" + response);

        // this is our for() loop iterating through the top ten events within the return
        // we are using their index within the response 
        for(var i = 0; i < 10; i++){

            // declaring a variable called eventId we are 
            var eventId = response.events.id;
